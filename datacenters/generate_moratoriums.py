#!/usr/bin/env python3
"""Generate the Maryland local data-center operating-environment layer.

County geometry comes from the existing Census TIGERweb-backed power estimate
layer. Policy status is curated from primary local government records, while
confirmed operating facilities come from the generated infrastructure inventory.
"""
...
DEFAULT_INFRASTRUCTURE = ROOT / "data" / "infrastructure.json"
...
ACTIONS = {
    "Baltimore County": {
        "policy_status": "restricted", "policy_status_label": "Restriction in effect",
    ...
    "Calvert County": {
        "policy_status": "pending_action", "policy_status_label": "Action pending",
...
def county_equivalent_name(raw_county: str) -> str:
    aliases = {
        "Baltimore": "Baltimore County",
        "Baltimore City": "Baltimore city",
        "Prince Georges": "Prince George's County",
        "Prince Geroges": "Prince George's County",
        "Queen Annes": "Queen Anne's County",
    }
    normalized = raw_county.strip().title() if raw_county.isupper() else raw_county.strip()
    return aliases.get(normalized, f"{normalized} County")


def confirmed_operating_counts(infrastructure_path: Path) -> dict[str, int]:
    records = json.loads(infrastructure_path.read_text())
    counts: dict[str, int] = {}
    for record in records:
        status = str(record.get("status") or "").strip().lower()
        if status != "operating" and not status.startswith("operating /"):
            continue
        county = county_equivalent_name(str(record.get("county") or ""))
        counts[county] = counts.get(county, 0) + 1
    return counts


def generate(geometry_path: Path, infrastructure_path: Path = DEFAULT_INFRASTRUCTURE) -> dict:
    geometry = json.loads(geometry_path.read_text())
    operating_counts = confirmed_operating_counts(infrastructure_path)
    features = []
    for source_feature in geometry["features"]:
        ...
        properties = {
            ...
            "status": "open", "status_label": "Open to data centers",
            "policy_status": "none", "policy_status_label": "No current action identified",
            "operating_facility_count": operating_counts.get(county, 0),
            "action_type": None, "legal_instrument": None, "adopted_date": None,
            "effective_date": None, "end_date": None,
            "scope": "No enacted moratorium, prohibition, pending formal action, or active administrative halt was identified in the reviewed local government records.",
            "source_title": None, "source_url": None, "verified_date": date.today().isoformat(),
        }
        properties.update(ACTIONS.get(county, {}))
        if properties["policy_status"] == "restricted":
            properties["status"] = "restricted"
            properties["status_label"] = "Moratorium or restriction"
        elif properties["policy_status"] == "pending_action":
            properties["status"] = "pending_action"
            properties["status_label"] = "Action pending"
        elif properties["operating_facility_count"]:
            properties["status"] = "operating"
            properties["status_label"] = "Data centers operating"
    ...
    counts = {status: sum(f["properties"]["status"] == status for f in features)
              for status in ("restricted", "pending_action", "operating", "open")}
    return {
        "type": "FeatureCollection",
        "metadata": {
            "title": "Maryland local data-center operating environment",
            "generated_date": date.today().isoformat(), "county_equivalent_count": len(features),
            "status_counts": counts,
            "status_definition": {
                "restricted": "A moratorium, prohibition, or administrative halt currently restricts new data-center activity.",
                "pending_action": "A formal restrictive action has been proposed but is not in effect.",
                "operating": "At least one confirmed operating facility is inventoried and no current restriction or pending action is identified.",
                "open": "No current restriction or pending action and no confirmed operating facility are identified.",
            },
            ...
            "policy_method": "Restrictions and proposals are curated from primary local government records. Operating status is derived from confirmed operating records in infrastructure.json. Restrictions take precedence over pending action, then operating presence, then open status.",
        },
        "features": features,
    }
...
def main() -> None:
    ...
    result = generate(args.geometry, args.infrastructure)
